...
while True:
    ...
    for i in range(4):
        ...
        if i != 3:
            dest_ip += '.'
    dst_port = int.from_bytes(text[4:6], byteorder='big')  # extracting the port from the message
    text = text[6:]  # without port

    mutex.acquire()
    messages.append([dest_ip, dst_port, text])  # insert the [ip, port, message] to the list
    mutex.release()
